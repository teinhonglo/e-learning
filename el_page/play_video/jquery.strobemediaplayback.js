...
(function($, undefined){
    ...
    var adaptiveExperienceConfiguratorMethods = {
        ...
        adapt: function(options){
			...
			//##
			//options.src = "videos/oceans.mp4";
			//alert("adapt:"+options.src);
    ...
    var strobeMediaPlaybackMethods = {
        initialize: function() {
            ...
            if (this.options.useHTML5) {
                ...
                $video.attr("video", this.options.src);
				...
				//##
				//alert("flashvars: "+flashvars.src);
				//alert("params: " +params);
				//flashvars.src = "videos/RayAllenTop10PlaysofCareer.mp4";
				//alert("flashvars: "+flashvars.src);
    ...
    $.fn.strobemediaplayback = function(options){
		//##
		//options.src = "videos/RayAllenTop10PlaysofCareer.mp4";
		//alert(options.src);
        var instances = [], i;
    ...
    $.fn.strobemediaplayback.generateQueryString = function(options){
		...
		* alert(options.src);
        ...
        for (var key in options) {
			...
			//## 取得真實路徑
			if(key == 'src'){
				var select_op = "" + options[key] + "PATH";
				var option = "";
				$.ajax({
					async: false,
					url: "video.php",
...
org.strobemediaplayback.triggerHandler = function(id, eventName, updatedProperties){
	alert("Cannot Found Video or Invalid Viewer Time");
